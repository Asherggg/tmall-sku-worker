...
- 源码直接启动默认 `demo`；v0.1.16 Windows 安装版由 Tauri host 显式启用 `tmall-publish-v2`，桌面 UI 只提供线上模式，批次预览列表在固定高度区域内滚动。适配器按服务端表单能力识别标准 SKU、SKU 明细和自定义销售属性异步校验，不按商品 ID 硬编码；最终回读允许平台数据在更长窗口内收敛，并只忽略可证明为空的笛卡尔积预检占位行。
