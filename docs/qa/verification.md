...
- Automated suite: 39/39 passed for v0.1.13, including inventory-only drift acceptance while retaining strict checks for every other SKU business field.
...
- The v0.1.13 NSIS installer builds at `src-tauri/target/release/bundle/nsis/Tmall SKU Worker_0.1.13_x64-setup.exe`.
...
The v0.1.13 source exposes only live mode in the desktop UI, retains an explicit confirmation dialog without typed confirmation, and limits the expanded batch preview to 10 items inside a fixed-height scrolling region. Final equivalence ignores `skuStock` only; SKU count, IDs, sale properties, price, merchant code, barcode, images, and other business fields remain strict. Worker tests continue to cover the direct submit contract, readback, item locks, field comparison, deletion audit behavior, and explicit confirmation before releasing an unresolved write lock.
