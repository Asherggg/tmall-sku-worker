...
| 需求状态 | v0.1.20 桌面 UI 仅提供线上模式；线上商品任务包含 OMS 禁用、Doris 料号资料补全和国补模板提交阶段；最终回读使用 150 秒截止时间并在截止点做最后一次新鲜 GET；登录检查同时验证淘宝、OMS 和国补页面；远程资料接口从应用数据目录运行时配置读取；未知外部写入统一进入人工复核 |
