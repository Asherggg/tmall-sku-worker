...
| 需求状态 | v0.1.23 桌面 UI 仅提供线上模式；商品资料接口、访问凭据和国补开关随安装版提供，用户只需完成三个站点登录；线上商品任务包含 OMS 禁用、Doris 料号资料补全、两阶段 SKU 重建和国补模板提交阶段；国补默认复用已登录 Edge 的 MTop/OSS HTTP 上下文并保留显式页面 fallback；未知外部写入统一进入人工复核 |
...
> 当前交付包含可运行的 Tauri/Vue 控制台、Playwright Profile 生命周期和 `tmall-publish-v2` 纯接口线上适配器。桌面 UI 只提供线上模式；线上执行采用两次提交和两次回读，任何未知响应或字段不一致都会进入人工复核，不能把请求已发出等同于成功。
...
3. 用户查看最多展开 10 个商品的批次预览，并在一次线上写入确认对话框中确认范围。
4. Worker 对每个商品串行执行“OMS 查询/禁用/回读 -> Doris 资料解析 -> 快照 -> 临时规格提交 -> 回读新 ID -> 恢复原数据提交 -> 最终回读”。
5. Worker 使用旧/新 SKU 映射生成国补模板并提交；用户在任务列表和审计详情中查看成功、失败和待人工复核项，可安全重试尚未发出外部写入的任务。
...
- 单 Worker 任务队列；线上任务启动后必须连续完成恢复与回读。
- OMS 商品禁用及禁用后回读。
- Doris 料号、69 码、品名和规格查询。
- 商品级两阶段 SKU 重建及提交后服务端回读。
- 国补 XLSX 模板通过已登录 Edge 的 HTTP 请求上下文导出、填充和上传；`IMPORT` 使用已验证的 MTop 契约，提交后按 SKU ID、69 码、国补品名和规格完成服务端列表回读。页面流程仅作显式 fallback。
