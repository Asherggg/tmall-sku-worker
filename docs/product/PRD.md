...
| 需求状态 | v0.1.21 桌面 UI 仅提供线上模式；线上商品任务包含 OMS 禁用、Doris 料号资料补全、两阶段 SKU 重建和国补模板提交阶段；国补默认复用已登录 Edge 的 MTop/OSS HTTP 上下文并保留显式页面 fallback；最终回读使用服务端证据；未知外部写入统一进入人工复核 |
