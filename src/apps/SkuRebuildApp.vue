...
import type { BatchRecord, ChannelOption, ImportPreview, TaskRecord, WorkerHealth } from "../types";
...
async function refresh(silent = false) {
  ...
  try {
    const [nextHealth, nextTasks, nextBatches] = await Promise.all([getHealth(), getTasks("sku_rebuild"), getBatches("sku_rebuild")]);
    health.value = nextHealth;
    tasks.value = nextTasks;
    batches.value = nextBatches;
    emit("status", {
      health: nextHealth,
      runningCount: nextTasks.filter((task) => ["queued", "reading_snapshot", "temp_submitting", "temp_verified", "restoring", "final_verifying"].includes(task.status)).length,
      reviewCount: nextTasks.filter((task) => ["needs_manual_review", "failed"].includes(task.status)).length,
...
async function handleBrowser(action: "login" | "hide" | "verify") {
  try {
    ...
    else ElMessage.success(action === "login" ? "已打开未附加自动化的专属 Edge" : action === "hide" ? "Edge 保持有头运行，窗口已隐藏" : health.value.message || (health.value.webdriver === false ? "登录态有效，webdriver 未开启" : "登录态检查完成"));
  } catch (error) {
...
function statusLabel(status: TaskRecord["status"]) {
  const labels: Record<TaskRecord["status"], string> = {
    draft: "草稿", validated: "已校验", planned: "已计划", awaiting_confirmation: "待确认", queued: "排队中", reading_snapshot: "读取快照", temp_submitting: "临时提交", temp_verified: "临时已回读", restoring: "恢复原数据", pattern_preparing: "准备花型", pattern_submitting: "提交花型", pattern_verifying: "回读花型", final_verifying: "最终回读", succeeded: "已完成", paused: "已暂停", needs_manual_review: "人工复核", failed: "失败",
  };
  ...
  <div class="app-shell">
    <aside class="sidebar">
      <div class="brand-lockup">
        ...
        <div><strong>SKU ID 重建</strong><span>天猫运营工作台应用</span></div>
      </div>
    ...
    <main class="main-area">
      ...
      <section v-if="view === 'overview'" class="content-view">
        ...
        <div class="metric-grid">
          ...
          <div class="metric"><span>处理中</span><strong>{{ runningTasks.length }}</strong><small>最多 {{ health.liveBatchConcurrency || 1 }} 路，提交串行</small></div>
      ...
      <section v-else-if="view === 'queue'" class="content-view">
        ...
        <div class="queue-toolbar"><span>{{ tasks.length }} 个商品任务</span><span class="toolbar-divider" /><span>跨商品最多 {{ health.liveBatchConcurrency || 1 }} 路，同商品不并发写入</span><span class="toolbar-spacer" /><el-tag v-if="health.unresolvedLiveWrites" type="danger" size="small">{{ health.unresolvedLiveWrites }} 个线上写入待核对</el-tag><el-tag v-else-if="health.contract === 'missing'" type="warning" size="small">线上适配器未配置</el-tag></div>
    ...
    <el-drawer v-model="drawerOpen" title="任务详情" size="460px" destroy-on-close>
      <template v-if="selectedTask">
        ...
        <div v-if="needsChannelMigration(selectedTask)" class="channel-drawer-action"><CircleAlert :size="16" /><div><strong>需要选择新的销售渠道</strong><p>旧值“{{ selectedTask.channelOptionObserved || '空值' }}”已被平台废弃。</p></div><button class="primary-button" @click="handleRetry(selectedTask)"><RotateCcw :size="15" />选择并重试</button></div>
