...
async function copyPackage(name, fromDirectory = root) {
  ...
  if (copiedPackages.has(realSource)) return;
...
for (const packageName of ["playwright", "playwright-core"]) await copyPackage(packageName);

const nodeBytes = (await fs.stat(path.join(resourcesDir, "node.exe"))).size;
console.log(`Prepared bundled Node runtime (${Math.round(nodeBytes / 1024 / 1024)} MB) and ${copiedPackages.size} browser runtime packages.`);
